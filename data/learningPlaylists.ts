 ...
 */
export const getFilteredPlaylists = (filters: { difficulty?: string; role?: string }): LearningPlaylist[] => {
  return LEARNING_PLAYLISTS.filter(playlist => {
    if (filters.difficulty && playlist.difficulty !== filters.difficulty) {
      return false;
    }
    if (filters.role && playlist.role !== filters.role) {
      return false;
    }
    return true;
  });
