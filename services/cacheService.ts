...
interface CacheEntry<T> {
  ...
  staleAt?: number; // Timestamp when data becomes stale (but still usable)
}
...
class CacheService {
   ...
   * Returns stale data immediately if available, then revalidates in background
   */
  async getWithRevalidate<T>(
    key: string,
    fetcher: () => Promise<T>,
    config: CacheConfig & {
      staleTtl?: number; // How long before data becomes stale (default: 80% of ttl)
      onUpdate?: (data: T) => void; // Callback when fresh data is available
    } = {}
  ): Promise<T> {
    const { version = '1.0', namespace = 'default', ttl, staleTtl, onUpdate } = config;
    const fullKey = this.buildKey(key, namespace);
    const effectiveTtl = ttl || this.DEFAULT_TTL;
    const effectiveStaleTtl = staleTtl || Math.floor(effectiveTtl * 0.8);

    // Try to get cached data
    const cached = this.get<T>(key, { version, namespace, ttl: effectiveTtl });

    if (cached) {
      // Check if stale
      try {
        const cachedEntry = localStorage.getItem(fullKey);
        if (cachedEntry) {
          const entry = JSON.parse(cachedEntry) as CacheEntry<T>;
          const age = Date.now() - entry.timestamp;
          const isStale = age >= effectiveStaleTtl;

          if (isStale) {
            console.debug(
              `[Cache] STALE: ${fullKey} (age: ${age}ms, staleTtl: ${effectiveStaleTtl}ms), revalidating...`
            );
            // Revalidate in background (don't await)
            this.revalidate(key, fetcher, { version, namespace, ttl: effectiveTtl, onUpdate }).catch(err => {
              console.error(`[Cache] Revalidation failed for ${fullKey}:`, err);
            });
          }
        }
      } catch (error) {
        console.error(`[Cache] Error checking staleness for ${fullKey}:`, error);
      }
    ...
    console.debug(`[Cache] MISS: ${fullKey}, fetching...`);
    const freshData = await fetcher();
    this.set(key, freshData, { version, namespace, ttl: effectiveTtl });
    return freshData;
  }
  ...
  private async revalidate<T>(
    key: string,
    fetcher: () => Promise<T>,
    config: CacheConfig & { onUpdate?: (data: T) => void } = {}
  ): Promise<void> {
    const { version = '1.0', namespace = 'default', ttl, onUpdate } = config;

    try {
      const freshData = await fetcher();
      this.set(key, freshData, { version, namespace, ttl });
      console.debug(`[Cache] REVALIDATED: ${this.buildKey(key, namespace)}`);
      ...
      console.error(`[Cache] Revalidation failed for ${this.buildKey(key, namespace)}:`, error);
      // Don't throw - we still have stale data to use
    }
  }
  ...
  set<T>(key: string, data: T, config: CacheConfig = {}): void {
    ...
    try {
      const entry: CacheEntry<T> = {
        ...
        data,
        ttl: effectiveTtl,
        staleAt: Date.now() + Math.floor(effectiveTtl * 0.8), // Stale at 80% of TTL
      };
      ...
      if (error instanceof DOMException && error.name === 'QuotaExceededError') {
        ...
        try {
          const entry: CacheEntry<T> = {
            ...
            data,
            ttl: effectiveTtl,
            staleAt: Date.now() + Math.floor(effectiveTtl * 0.8),
          };
          localStorage.setItem(fullKey, JSON.stringify(entry));
        } catch (retryError) {
