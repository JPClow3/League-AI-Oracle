...
export const ProfileSetupModal = ({ isOpen, onComplete }: ProfileSetupModalProps) => {
  ...
  const toggleFavorite = (id: string) => {
    setFavoriteChampions(prev => (prev.includes(id) ? prev.filter(champId => champId !== id) : [...prev, id]));
      ...
      <div className="flex flex-col h-[70vh]">
            ...
            >
              ...
              {step === 2 && (
                <div>
                  ...
                  <p className="text-text-secondary text-center mb-6">
                    This helps us tailor suggestions, but you can always change it later.
                  </p>
              ...
              {step === 3 && (
                <div>
                  ...
                  <p className="text-text-secondary text-center mb-4">
                    Select a few of your favorites to get a head start on mastery. This is optional!
                  </p>
                  <input
                    type="text"
                    value={champSearch}
                  ...
                  <div className="max-h-60 overflow-y-auto flex flex-wrap gap-2 p-2 bg-surface-inset border border-border">
                    {filteredChamps.map(champ => (
                      <button
                        key={champ.id}
                        onClick={() => toggleFavorite(champ.id)}
                        className={`p-1 rounded-lg ${favoriteChampions.includes(champ.id) ? 'ring-2 ring-accent' : ''}`}
                      >
                    ...
                    <div className="flex flex-col gap-3">
                      {(['Beginner', 'Intermediate', 'Advanced'] as const).map(level => (
                        <button
                          ...
                          className={`p-3 border rounded-lg text-left ${skillLevel === level ? 'bg-accent/10 border-accent' : 'bg-surface hover:border-border'}`}
                        >
                    ...
                    <div className="flex flex-col gap-3">
                      {GOALS.map(goal => (
                        <button
                          ...
                          className={`p-3 border rounded-lg text-left flex items-center gap-3 ${goals.includes(goal) ? 'bg-accent/10 border-accent' : 'bg-surface hover:border-border'}`}
                        >
                          <div
                            className={`w-5 h-5 border-2 rounded-sm flex items-center justify-center ${goals.includes(goal) ? 'bg-accent border-accent' : 'border-border'}`}
                          >
                            {goals.includes(goal) && (
                              <motion.div
                                {...{ initial: { scale: 0 }, animate: { scale: 1 } }}
                                className="w-3 h-3 bg-on-accent rounded-sm"
                              />
                            )}
                          </div>
              ...
              {step === 5 && (
                <div>
                  ...
                  <p className="text-text-secondary text-center mb-4">
                    This will represent you on your profile. Click to select.
                  </p>
                  <div className="max-h-80 overflow-y-auto flex flex-wrap gap-2 p-2 bg-surface-inset border border-border">
                    {championsLite.map(champ => (
                      <button
                        key={champ.id}
                        onClick={() => setAvatar(champ.id)}
                        className={`p-1 rounded-lg ${avatar === champ.id ? 'ring-2 ring-accent' : ''}`}
                      >
        ...
        <div className="p-4 border-t border-border flex justify-between items-center flex-shrink-0">
          ...
          <div className="flex gap-2">
            ...
            {step < TOTAL_STEPS && (
              <Button variant="primary" onClick={nextStep} disabled={isNextDisabled()}>
                Next
              </Button>
            )}
            {step === TOTAL_STEPS && (
              <Button variant="primary" onClick={handleComplete} disabled={isNextDisabled()}>
                Finish Setup
              </Button>
            )}
          </div>
