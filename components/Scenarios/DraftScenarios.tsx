        ...
        <div className="space-y-6">
            ...
            <div className="bg-surface p-6 shadow-lg border border-border space-y-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <TeamPanel side="blue" state={draftState.blue} onSlotClick={() => {}} activeSlot={currentScenario.userContext.side === 'blue' ? {type: 'pick', index: currentScenario.userContext.index} : null} />
                    <TeamPanel side="red" state={draftState.red} onSlotClick={() => {}} activeSlot={currentScenario.userContext.side === 'red' ? {type: 'pick', index: currentScenario.userContext.index} : null} />
                </div>
            </div>
                           ...
                           {selectedOption && (
                                <div className="text-center">
                                    <div className="flex justify-center items-center gap-3 mb-3">
                                        {selectedOption.isCorrect ? <Check size={32} className="text-success" /> : <X size={32} className="text-error" />}
                                        <h3 className={`text-3xl font-bold font-display ${selectedOption.isCorrect ? 'text-success' : 'text-error'}`}>
