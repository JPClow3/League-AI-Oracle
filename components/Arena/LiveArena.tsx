...
const BOT_PERSONAS: ArenaBotPersona[] = ['The Strategist', 'The Aggressor', 'The Trickster', 'The Meta Slave', 'The One-Trick', 'The Chaos Draft', 'The Guardian', 'The Split Pusher'];
...
const ArenaResults = ({ analysis, userSide, onReset, onNavigateToForge }: { analysis: AIAdvice | null, userSide: TeamSide, onReset: () => void, onNavigateToForge: () => void }) => {
    if (!analysis) {
        return <div className="text-center bg-surface p-8 border border-border"><Loader messages={['Analyzing final score...']} /></div>;
    }
    ...
    return (
        <div className="text-center bg-surface p-8 border border-border space-y-4">
            ...
            <div className="flex flex-wrap justify-center gap-4">
                <Button onClick={onNavigateToForge} variant="primary">Analyze in Lab</Button>
                <Button onClick={onReset} variant="secondary">New Arena Draft</Button>
            </div>
        </div>
    );
};
...
export const LiveArena = ({ draftState, setDraftState, onReset, onNavigateToForge }: LiveArenaProps) => {
    ...
    const timer = setTimeout(async () => {
      ...
      if (botChampion) {
          if (reasoning && !reasoning.startsWith('Failsafe:')) {
              ...
              toast.custom((t) => (
                <div className={`${t.visible ? 'animate-enter' : 'animate-leave'} max-w-sm w-full bg-surface shadow-lg pointer-events-auto flex ring-1 ring-error/30 border border-border`}>
  ...
  return (
    <div className="space-y-6">
        <QuickLookPanel champion={quickLookChampion} onClose={() => setQuickLookChampion(null)} onDraft={handleChampionSelectLite} canDraft={isModalOpen || isOneTrickModalOpen} />
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-surface p-4 shadow-lg">
             <div className="flex items-center gap-4">
                <div className="bg-secondary text-accent w-12 h-12 flex items-center justify-center flex-shrink-0"><Swords size={32} /></div>
                <div>
            ...
            {isDraftStarted && (
                 <div className="flex flex-wrap gap-2">
                    <Button onClick={() => setIsSaveModalOpen(true)} variant="secondary" disabled={!draftFinished}>Save to The Archives</Button>
        ...
        {!isDraftStarted ? (
            <div className="bg-surface p-6 sm:p-8 border border-border text-center">
                ...
                <p className="text-text-secondary mb-6 max-w-xl mx-auto">Choose your side and select an AI opponent to begin your draft practice.</p>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                    <div className="bg-secondary p-4 rounded-lg border border-border">
                        <h3 className="text-lg font-bold text-text-primary mb-3">1. Choose Your Side</h3>
                        <div className="flex gap-2">
                            <Button onClick={() => setUserSide('blue')} variant={userSide === 'blue' ? 'primary' : 'secondary'} className="w-full !py-3">Blue Side</Button>
                            <Button onClick={() => setUserSide('red')} variant={userSide === 'red' ? 'primary' : 'secondary'} className="w-full !py-3">Red Side</Button>
                        </div>
                    </div>
                     <div className="bg-secondary p-4 rounded-lg border border-border">
                        <h3 className="text-lg font-bold text-text-primary mb-3">2. Choose Your Opponent</h3>
                        <select onChange={e => setBotPersona(e.target.value as ArenaBotPersona)} value={botPersona} className="w-full bg-surface-inset border border-border p-2 focus:outline-none focus:ring-2 focus:ring-accent">
                            {BOT_PERSONAS.map(p => <option key={p} value={p}>{p}</option>)}
                        </select>
                        <p className="text-xs text-text-secondary italic mt-2 h-8">{personaDescriptions[botPersona]}</p>
                        {botPersona === 'The One-Trick' && (
                            <div className="flex items-center gap-2 mt-2">
                                <Button onClick={() => setIsOneTrickModalOpen(true)} variant="secondary">Select Champion</Button>
                                {oneTrickChampion && <div className="flex items-center gap-2"><img src={oneTrickChampion.image} className="w-8 h-8" alt={oneTrickChampion.name}/> <span className="font-semibold">{oneTrickChampion.name}</span></div>}
                            </div>
                        )}
                    </div>
                </div>
                <div className="mt-8">
                     <Button onClick={() => setCurrentTurnIndex(0)} variant="primary" className="!text-lg !px-8 !py-4" disabled={botPersona === 'The One-Trick' && !oneTrickChampion}>
                        Start Draft
                    </Button>
                </div>
            </div>
        ...
        {draftFinished && (
            <ArenaResults analysis={finalAnalysis} userSide={userSide} onReset={handleReset} onNavigateToForge={() => onNavigateToForge(draftState)} />
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <TeamPanel side="red" state={draftState.red} onSlotClick={handleSlotClick} activeSlot={activeSlotForTeam('red')} isTurnActive={!draftFinished && currentTurn?.team === 'red'} onDragStart={handleDragStart} onDrop={handleDrop} onDragOver={(e) => e.preventDefault()} />
        </div>

        <ArenaChampionSelectModal isOpen={isModalOpen || isOneTrickModalOpen} onClose={() => {setIsModalOpen(false); setIsOneTrickModalOpen(false);}} onSelect={handleChampionSelectLite} onQuickLook={setQuickLookChampion} draftState={draftState} title={isOneTrickModalOpen ? 'Select One-Trick Champion' : 'Select your Champion'}/>
        <ArenaSaveModal isOpen={isSaveModalOpen} onClose={() => setIsSaveModalOpen(false)} draftState={draftState} botPersona={botPersona} userSide={userSide} />
    </div>
  );
};