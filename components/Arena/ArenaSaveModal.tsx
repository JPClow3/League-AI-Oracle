...
export const ArenaSaveModal = ({ isOpen, onClose, draftState, botPersona, userSide }: ArenaSaveModalProps) => {
  ...
  const handleConfirmSave = async () => {
    if (draftName.trim() && !isSaving) {
      setIsSaving(true);
      const success = await addPlaybookEntry(
        draftName.trim(),
        draftState,
        null,
        `Practice draft against the ${botPersona} AI.`,
        userSide,
        draftTags
      );
      setIsSaving(false);
  ...
  return (
    ...
    >
      <div className="p-6 space-y-4">
        <div>
          <label htmlFor="draftName" className="block text-sm font-medium text-text-secondary">
            Draft Name
          </label>
          <input
            id="draftName"
        ...
        <div>
          <label htmlFor="draftTags" className="block text-sm font-medium text-text-secondary">
            Tags
          </label>
