...
const getAnalysisTitle = (score: string | undefined): { title: string; className: string } => {
  if (!score) {
    return { title: 'Draft Analysis', className: 'text-text-primary' };
  }
  if (score.startsWith('S')) {
    return { title: 'Masterful Composition', className: 'text-success' };
  }
  if (score.startsWith('A')) {
    return { title: 'Excellent Composition', className: 'text-info' };
  }
  if (score.startsWith('B')) {
    return { title: 'Solid Foundation', className: 'text-gold' };
  }
  if (score.startsWith('C')) {
    return { title: 'Room for Improvement', className: 'text-warning' };
  }
...
const GradeDisplay = ({ score }: { score: string }) => {
  ...
  return (
    <div className={`relative flex flex-col items-center justify-center p-4 min-h-[120px] ${gradeColor}`}>
      ...
      >
        <span className="font-display font-black text-7xl" style={{ textShadow: '0 0 10px currentColor' }}>
          {grade}
        </span>
...
const TabButton = ({
  ...
  <button
    ...
    className={`relative px-4 py-2 font-semibold transition-colors ${active ? 'text-text-primary' : 'text-text-secondary hover:text-text-primary'}`}
  >
    {children}
    {active && (
      <motion.div
        {...{ layoutId: 'advice-tab-underline' }}
...
const KeyMatchupCard = ({ matchup }: { matchup: NonNullable<TeamAnalysis['keyMatchups']>[0] }) => {
  ...
  return (
    <div className="bg-surface-primary p-4 border border-border">
      <h4 className="font-semibold text-text-secondary text-center mb-2 uppercase text-xs tracking-wider">
        {matchup.role}
      </h4>
      ...
      <p className="text-xs text-text-secondary mt-2 text-center italic">
        &quot;
        <KeywordHighlighter text={matchup.analysis} />
        &quot;
      </p>
    </div>
  );
...
const SynergyCard = ({
  ...
  synergy: NonNullable<TeamAnalysis['synergies']>[0];
  type: 'good' | 'bad';
}) => {
    ...
    <div className="flex items-start gap-3 text-sm bg-surface p-3 border border-border">
      ...
      <div>
        ...
        <p className="text-text-secondary text-xs italic">
          &quot;
          <KeywordHighlighter text={synergy.reasoning} />
          &quot;
        </p>
      </div>
    </div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="space-y-4">
        ...
        <div className="bg-surface p-3 text-center text-sm italic text-text-secondary">
          {analysis.draftScoreReasoning}
        </div>

        <div className="bg-surface p-4 border border-border space-y-2">
          <h3 className="font-semibold text-lg text-accent">Summary</h3>
          <p className="text-sm">
            <strong className="font-semibold text-text-primary">Identity:</strong>{' '}
            <KeywordHighlighter text={analysis.teamIdentity} onKeywordClick={navigateToAcademy} />
          </p>
          <p className="text-sm">
            <strong className="font-semibold text-text-primary">Win Condition:</strong>{' '}
            <KeywordHighlighter text={analysis.winCondition} onKeywordClick={navigateToAcademy} />
          </p>
        </div>
      </div>
      <div className="space-y-4">
        ...
        <div className="md:col-span-2 mt-4 p-4 bg-secondary/50 rounded-lg border border-accent/20">
          ...
          <ul className="space-y-2">
            {improvementOpportunities.map(({ keyword, lessonId }) => (
              <li
                key={keyword}
                className="flex flex-col sm:flex-row justify-between items-start sm:items-center text-sm bg-surface/50 p-2 rounded-md"
              >
                <p className="mb-2 sm:mb-0">
                  Your draft shows a weakness in: <strong className="text-text-primary">{keyword}</strong>
                </p>
                <Button variant="secondary" onClick={() => navigateToAcademy(lessonId)}>
            ...
            <div className="space-y-2">
              {(analysis.synergies || []).map((s, i) => (
                <React.Fragment key={`syn-${i}`}>
                  <SynergyCard synergy={s} type="good" />
                </React.Fragment>
              ))}
            </div>
            <div className="space-y-2">
              {(analysis.antiSynergies || []).map((s, i) => (
                <React.Fragment key={`asyn-${i}`}>
                  <SynergyCard synergy={s} type="bad" />
                </React.Fragment>
              ))}
            </div>
          </div>
...
const HeadToHeadContent = ({
  ...
  return (
    <div className="space-y-6">
      {advice.teamAnalysis.blue.powerSpikeTimeline && (
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {advice.teamAnalysis.blue.keyMatchups.map((matchup, i) => (
              <React.Fragment key={i}>
                <KeyMatchupCard matchup={matchup} />
              </React.Fragment>
            ))}
          </div>
        </div>
              ...
              {advice.pickSuggestions.slice(0, 3).map(p => (
                <div key={p.championName} className="bg-surface p-3 border border-border group relative">
                  ...
                  <p className="text-xs text-text-secondary">
                    <KeywordHighlighter text={p.reasoning} onKeywordClick={navigateToAcademy} />
                  </p>
                </div>
              ))}
            </div>
...
const BuildSuggestionsContent = ({ builds }: { builds: NonNullable<AIAdvice['buildSuggestions']> }) => {
  if (!builds || builds.length === 0) {
    return (
      <div className="text-center p-8 text-text-secondary">No specific build suggestions available for this draft.</div>
    );
  }
      ...
      {builds.map(build => (
        <div key={build.championName} className="bg-surface p-4 border border-border space-y-4">
          ...
          <p className="text-sm italic text-text-secondary">
            &quot;
            <KeywordHighlighter text={build.reasoning} />
            &quot;
          </p>
          ...
          <div>
            ...
            <div className="bg-secondary p-3 text-sm rounded-md border border-border-secondary">
              <p>
                <strong className="font-semibold">{build.runes.primaryPath}:</strong> {build.runes.keystone}
              </p>
              <p>
                <strong className="font-semibold">{build.runes.secondaryPath}</strong>
              </p>
            </div>
          </div>
          <div>
            ...
            <div className="space-y-2 text-sm">
              <p>
                <strong className="font-semibold text-text-secondary">Core:</strong> {build.coreItems.join(', ')}
              </p>
              <div>
                ...
                <ul className="list-disc list-inside text-text-secondary">
                  {build.situationalItems.map(item => (
                    <li key={item.item}>
                      <strong className="text-text-primary">{item.item}:</strong> {item.reason}
                    </li>
                  ))}
                </ul>
              </div>
