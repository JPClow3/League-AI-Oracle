...
export const TeamBuilderAssistant = ({ step, coreConcept, suggestions, isLoading, onSelect, onGenerateOpponent, isOpponentLoading }: TeamBuilderAssistantProps) => {
    ...
    if (step >= 5) {
        return (
            <div className="bg-surface-secondary border border-border-primary p-6 h-full flex flex-col items-center justify-center text-center">
                 ...
                 <p className="text-text-secondary mb-6">You've built a "{coreConcept}" composition. Now, generate an opponent team to analyze the matchup.</p>
                 <Button onClick={onGenerateOpponent} disabled={isOpponentLoading} variant="primary" className="text-lg py-3 px-6">
                    {isOpponentLoading ? 'Generating...' : <><Swords size={20} className="mr-2"/> Generate Opponent</>}
                 </Button>
            </div>
        )
    }
    ...
    return (
        <div className="bg-surface-secondary border border-border-primary p-4 h-full flex flex-col">
            <div className="text-center pb-3 border-b border-border mb-3">
                ...
                <p className="text-sm text-accent font-semibold">Step {step + 1}/5: Selecting {currentRole}</p>
                 <p className="text-xs text-text-secondary mt-1">Core Concept: <span className="font-bold">"{coreConcept}"</span></p>
            </div>
