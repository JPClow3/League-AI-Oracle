...
const ResultIcon = ({ result }: { result: HistoryEntry['result'] }) => {
  ...
  return (
    <Tooltip content={`Result: ${result.charAt(0).toUpperCase() + result.slice(1)}`}>
      <div
        className={`absolute top-2 right-2 w-6 h-6 rounded-full flex items-center justify-center bg-surface border border-border`}
      >
        {iconMap[result]}
      </div>
    </Tooltip>
...
export const PlaybookCard = memo(({ entry, onViewDetails, onSelectForCompare, isSelected }: PlaybookCardProps) => {
      ...
      if (!id) {
        return (
          <div
            ...
            className="w-8 h-8 bg-surface-inset border-2 border-border-primary rounded-full"
          ></div>
        );
      }
        ...
        {entry.tags && entry.tags.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {entry.tags.slice(0, 3).map(tag => (
              <span key={tag} className="text-xs bg-surface-tertiary px-2 py-0.5 text-text-secondary">
                {tag}
              </span>
            ))}
          </div>
