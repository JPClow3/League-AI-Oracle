...
export const CommandPalette = ({ isOpen, onClose, commands }: CommandPaletteProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const timeoutId = setTimeout(() => {
        setSearchTerm('');
        setActiveIndex(0);
        focusTimeoutId = setTimeout(() => inputRef.current?.focus(), 100);
      }, 0);
      return () => {
  ...
  const filteredCommands = useMemo(
    () => (searchTerm ? commands.filter(cmd => cmd.title.toLowerCase().includes(searchTerm.toLowerCase())) : commands),
    [commands, searchTerm]
  );

  const groupedCommands = useMemo(
    () =>
      filteredCommands.reduce<Record<string, Command[]>>(
        (acc, cmd) => {
      ...
      >
        ...
        >
          ...
          <ul ref={resultsRef} id="command-list" role="listbox" className="max-h-[50vh] overflow-y-auto p-2">
            ...
            {flatCommands.length === 0 && <li className="p-4 text-center text-text-secondary">No results found.</li>}
          </ul>
        </div>
      </FocusTrap>
    </div>
