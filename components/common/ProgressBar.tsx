...
interface ProgressBarProps {
  ...
  size?: 'sm' | 'md' | 'lg';
  className?: string;
  colorClass?: string; // optional override for inner bar color
}
    ...
    <div className={`w-full ${className}`}>
      {(label || showPercentage) && (
        <div className="flex justify-between items-center mb-2">
          ...
          {showPercentage && <span className="text-sm font-semibold text-text-primary">{percentage}%</span>}
        </div>
      )}
      <div
      ...
      >
        <div
          className={`${sizeClasses[size]} ${colorClass ?? variantClasses[variant]} rounded-full transition-all duration-300 ease-out`}
