    ...
    <>
      ...
      <div className="flex justify-center gap-3">
        <button
          ...
          className="p-2 rounded-full bg-secondary hover:bg-success/20 text-text-secondary hover:text-success transition-colors"
        >
          <ThumbsUp className="h-5 w-5" />
        </button>
        <button
          onClick={() => onFeedback('down')}
          className="p-2 rounded-full bg-secondary hover:bg-error/20 text-text-secondary hover:text-error transition-colors"
        >
...
export const Academy = ({ initialLessonId, onHandled, loadChampionsAndNavigateToForge }: AcademyProps) => {
  ...
  const staticLessons = useMemo(() => {
    ...
    return LESSONS.map(category => ({
      ...
      lessons: category.lessons.filter(
        lesson => lesson.title.toLowerCase().includes(lowerSearch) || lesson.content.toLowerCase().includes(lowerSearch)
      ),
  ...
  const selectedLesson = useMemo(() => {
    for (const category of LESSONS) {
      ...
      if (found) {
        return { ...found, isGenerated: false };
      }
    }
    const foundGenerated = generatedLessons.find(l => l.id === selectedLessonId);
    if (foundGenerated) {
      return { ...foundGenerated, isGenerated: true };
    }
    return null;
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4 bg-surface-primary border border-border-primary p-4 shadow-sm">
        ...
        <div>
          ...
          <p className="text-sm text-text-secondary">
            Your knowledge base for mastering high-level League of Legends strategy.
          </p>
        </div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-1 space-y-4">
          ...
          <Button
            variant="secondary"
            onClick={() => handleGenerateLesson(searchTerm || 'Custom Lesson')}
            ...
            >
              {selectedLesson ? (
                <div className="prose prose-sm md:prose-base dark:prose-invert max-w-none prose-headings:text-accent prose-strong:text-text-primary">
                  ...
                  {selectedLesson.practiceChampionIds && (
                    <div className="not-prose my-4">
                      <Button
                        ...
                        onClick={() =>
                          selectedLesson.practiceChampionIds &&
                          loadChampionsAndNavigateToForge(selectedLesson.practiceChampionIds)
                        }
                      >
                  ...
                  {generationError && currentGenerationId === selectedLesson.id && (
                    <p className="text-error">{generationError}</p>
                  )}
