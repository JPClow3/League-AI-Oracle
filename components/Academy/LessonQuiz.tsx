...
export const LessonQuiz = ({ questions, onComplete }: LessonQuizProps) => {
  ...
  const handleNext = () => {
    if (isLastQuestion) {
      ...
      const score = Math.round(
        ((correctAnswers + (selectedAnswer === currentQuestion!.correctAnswer ? 1 : 0)) / questions.length) * 100
      );
      setFinalScore(score);
      setIsComplete(true);
      onComplete(score);
  ...
  if (isComplete) {
    ...
    return (
      ...
      >
        <div
          className={`w-20 h-20 mx-auto mb-4 rounded-full flex items-center justify-center ${passed ? 'bg-success/20' : 'bg-warning/20'}`}
        >
        ...
        <div className="text-5xl font-bold text-accent mb-6">{finalScore}%</div>
        ...
        <div className="text-sm text-text-secondary mb-6">
          Correct: {correctAnswers + (selectedAnswer === questions[questions.length - 1]?.correctAnswer ? 1 : 0)} /{' '}
          {questions.length}
        </div>
  ...
  return (
    <div className="p-6 bg-surface-primary border border-border-primary rounded-lg">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <h3 className="text-lg font-semibold text-text-primary mb-4">{currentQuestion.question}</h3>
          ...
          <div className="flex gap-3">
            {!showExplanation ? (
              <Button variant="primary" onClick={handleSubmit} disabled={selectedAnswer === null} className="flex-1">
                Submit Answer
              </Button>
            ) : (
