...
interface HomeProps {
  ...
  navigateToStrategyHub: (tab: 'champions' | 'intel') => void;
}
...
const FeatureCard: React.FC<{
  ...
  <button
    onClick={onClick}
    className="group relative bg-slate-800/50 backdrop-blur-sm border border-slate-700/80 rounded-2xl shadow-lg p-6 flex flex-col text-left hover:border-cyan-400/50 transition-all duration-300 transform hover:-translate-y-1 focus:outline-none focus:ring-4 focus:ring-cyan-500/30 w-full h-full"
  >
    {/* Glow effect */}
    ...
    <div className="relative z-10 flex-grow flex flex-col h-full">
        <div className="bg-slate-800/70 text-cyan-300 w-12 h-12 rounded-lg flex items-center justify-center mb-4 border border-slate-700 group-hover:border-cyan-400/50 group-hover:text-cyan-200 transition-colors duration-300">
            {icon}
        </div>
...
export const Home: React.FC<HomeProps> = ({ setCurrentPage, navigateToStrategyHub }) => {
  ...
  return (
    <div className="space-y-12">
      <div className="text-center p-8 md:p-12 bg-slate-900/50 border border-slate-700/50 rounded-2xl shadow-2xl relative overflow-hidden">
        ...
        <div className="relative">
            ...
            <p className="mt-4 max-w-2xl mx-auto text-lg text-gray-300">
              Welcome to DraftWise AI — Your ultimate strategic co-pilot for League of Legends.
            </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <FeatureCard
          title="Draft Lab"
          description="Theory-craft compositions, experiment with picks, and get instant AI-powered strategic analysis."
          icon={ICONS.DRAFT_LAB}
          actionText="Enter the Lab"
          onClick={() => setCurrentPage('Draft Lab')}
        />
        <FeatureCard
          title="Drafting Arena"
        ...
        <FeatureCard
          title="Playbook"
          description="Your personal library of strategies. Save drafts from the Lab or Arena and refine your master plans."
          icon={ICONS.PLAYBOOK}
          actionText="Open Playbook"
          onClick={() => setCurrentPage('Playbook')}
        />
        <FeatureCard
          title="Champion Armory"
          description="Explore detailed champion data, optimal builds, counters, and synergies to master your picks."
          icon={ICONS.ARMORY}
          actionText="Browse Champions"
          onClick={() => navigateToStrategyHub('champions')}
        />
        <FeatureCard
          title="AI Intel Hub"
          ...
          onClick={() => navigateToStrategyHub('intel')}
        />
        <FeatureCard
          title="Academy"
          description="Sharpen your strategic knowledge with curated lessons on drafting fundamentals and macro play."
          icon={ICONS.ACADEMY}
