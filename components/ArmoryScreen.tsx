        ...
        const fetchAnalysis = async () => {
            ...
            try {
                ...
                if (result) {
                    ...
                    setAnalysisError("The Oracle could not provide an analysis for this champion.");
                }
            } catch (e) {
                setAnalysisError((e as Error).message || "An unexpected error occurred.");
                console.error(e);
            } finally {
        ...
        <div className="space-y-6">
            ...
            {!vaultData ? (
                ...
                <div className="space-y-4 animate-fade-in">
                    <Section title="Overview" icon="brain" defaultOpen>
                        <p className="text-slate-600 dark:text-slate-300">{vaultData.overview}</p>
                    </Section>
                    
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Section title="Strengths" icon="check" defaultOpen>
                            <ul className="list-disc list-inside space-y-1 text-slate-600 dark:text-slate-300">
                                {vaultData.strengths.map(s => <li key={s}>{s}</li>)}
                            </ul>
                        </Section>
                        <Section title="Weaknesses" icon="x" defaultOpen>
                             <ul className="list-disc list-inside space-y-1 text-slate-600 dark:text-slate-300">
...
const BuildsAndAnalysisTabs: React.FC<{ 
    vaultData: ChampionVaultEntry, 
    ddragonData: DDragonData,
    analysis: SynergyAndCounterAnalysis | null,
    isLoading: boolean,
    error: string | null
}> = ({ vaultData, ddragonData, analysis, isLoading, error }) => {
    ...
    return (
        <div className="bg-slate-100/50 dark:bg-slate-900/30 rounded-lg p-4">
            <div className="border-b border-slate-200 dark:border-slate-700 flex mb-4">
                ...
                <TabButton name="Synergies & Counters" icon="lab" isActive={activeTab === 'analysis'} onClick={() => setActiveTab('analysis')} />
            </div>
            <div className="py-4">
                {activeTab === 'build' && (
                ...
                {activeTab === 'analysis' && (
                    <div className="animate-fade-in">
                        {isLoading && <div className="flex justify-center p-8"><Spinner size="h-8 w-8"/></div>}
                        {error && <div className="p-4 text-center text-rose-500">{error}</div>}
                        {analysis && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <AnalysisColumn title="Top Synergies" champions={analysis.synergies} ddragonData={ddragonData} color="teal" />
                                <AnalysisColumn title="Top Counters" champions={analysis.counters} ddragonData={ddragonData} color="rose" />
                            </div>
                        )}
                    </div>
                )}
...
const AnalysisColumn: React.FC<{title: string, champions: {championName: string, reasoning: string}[], ddragonData: DDragonData, color: 'teal' | 'rose'}> = ({ title, champions, ddragonData, color }) => (
    <div>
        <h4 className={`font-bold text-${color}-600 dark:text-${color}-400 mb-2 text-lg`}>{title}</h4>
        <div className="space-y-2">
            {champions.map(({ championName, reasoning }) => {
                ...
                return (
                    <div key={championName} className="flex items-start gap-3 p-2 bg-slate-200/50 dark:bg-black/20 rounded-md">
                        <ChampionIcon champion={champion} version={ddragonData.version} className="w-12 h-12 flex-shrink-0" isClickable={false} />
                        <div>
                             <p className="font-bold text-slate-800 dark:text-slate-200">{championName}</p>
                            <p className="text-xs text-slate-600 dark:text-slate-400">{reasoning}</p>
                        </div>
                    </div>
                )
            })}
...
const ItemIcon: React.FC<{item: Item; version: string}> = ({ item, version }) => (
    <div className="flex flex-col items-center gap-1 w-20 text-center" title={`${item.name} - ${item.plaintext}`}>
        <img src={`https://ddragon.leagueoflegends.com/cdn/${version}/img/item/${item.image.full}`} alt={item.name} className="w-16 h-16 rounded-md border-2 border-slate-400 dark:border-slate-600" loading="lazy" decoding="async" width="64" height="64"/>
        <p className="text-xs text-slate-500 dark:text-slate-400 truncate w-full">{item.name}</p>
    </div>
);

const Section: React.FC<{title: string; icon: React.ComponentProps<typeof Icon>['name']; children: React.ReactNode, defaultOpen?: boolean}> = ({ title, icon, children, defaultOpen = false }) => (
    <details className="bg-slate-100/50 dark:bg-slate-800/80 rounded-lg border border-slate-200/50 dark:border-slate-700/50" open={defaultOpen}>
        <summary className="p-4 flex items-center gap-2 text-xl font-display text-slate-700 dark:text-slate-300 cursor-pointer">
