...
export const PersonalizedPatchNotesDisplay = ({ summary, isLoading, error, onRetry }: PersonalizedPatchNotesDisplayProps) => {
    return (
        <div className="bg-gradient-to-br from-accent/5 to-surface/50 p-6 rounded-lg border-2 border-accent/20 shadow-lg">
            <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="font-display text-2xl font-bold text-text-primary tracking-wide">Your Personalized Patch Briefing</h2>
            </div>

            {isLoading && <div className="min-h-[150px]"><Loader messages={["Crafting your personal briefing..."]} /></div>}

            {error && !isLoading && (
                <div className="text-center text-error p-4">
                    <p>{error}</p>
                    {onRetry && <Button onClick={onRetry} variant="secondary" className="mt-2">Retry</Button>}
                </div>
            )}
                        ...
                        <div>
                            ...
                            {summary.relevantBuffs.length > 0 ? (
                                <div className="space-y-2">
                                    {summary.relevantBuffs.map(change => <React.Fragment key={change.name}><ChangeItem change={change} /></React.Fragment>)}
                                </div>
                            ) : (
                         ...
                         <div>
                            ...
                            {summary.relevantNerfs.length > 0 ? (
                                <div className="space-y-2">
                                    {summary.relevantNerfs.map(change => <React.Fragment key={change.name}><ChangeItem change={change} /></React.Fragment>)}
                                </div>
                             ) : (
