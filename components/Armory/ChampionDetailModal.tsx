  ...
  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
    <div className="md:col-span-1 space-y-4">
      ...
      <div className="space-y-2 bg-secondary p-3 rounded-md text-sm text-text-secondary">
        ...
        <p>
          <strong>Damage Type:</strong> {champion.damageType}
        </p>
        <p>
          <strong>Crowd Control:</strong> {champion.cc}
        </p>
        <p>
          <strong>Engage:</strong> {champion.engage}
        </p>
      </div>
    </div>
  ...
  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
    <div className="space-y-6">
      <Section title="Runes">
        <div className="bg-secondary p-4 rounded-lg border border-border">
          <h4 className="font-bold text-text-primary">
            {analysis.runes.primaryPath}: <span className="text-accent">{analysis.runes.primaryKeystone}</span>
          </h4>
          <ul className="list-disc list-inside text-sm text-text-secondary">
            {analysis.runes.primaryRunes.map(r => (
              <li key={r}>{r}</li>
            ))}
          </ul>
          <h4 className="font-bold text-text-primary mt-2">{analysis.runes.secondaryPath}</h4>
          <ul className="list-disc list-inside text-sm text-text-secondary">
            {analysis.runes.secondaryRunes.map(r => (
              <li key={r}>{r}</li>
            ))}
          </ul>
        </div>
      ...
      <Section title="Playstyle">
        <div className="space-y-2 text-sm">
          <p>
            <strong className="font-semibold text-text-primary">Early Game:</strong> {analysis.playstyle.earlyGame}
          </p>
          <p>
            <strong className="font-semibold text-text-primary">Mid Game:</strong> {analysis.playstyle.midGame}
          </p>
          <p>
            <strong className="font-semibold text-text-primary">Late Game:</strong> {analysis.playstyle.lateGame}
          </p>
        </div>
      </Section>
    </div>
    <div className="space-y-6">
        ...
        <div className="space-y-3">
          <div>
            ...
            <div className="flex flex-wrap gap-2 text-sm text-text-secondary">
              {analysis.build.startingItems.join(', ')}
            </div>
          </div>
          <div>
            <h4 className="font-semibold text-text-secondary mb-1">Core Items</h4>
            <div className="flex flex-wrap gap-2 text-sm text-accent font-bold">
              {analysis.build.coreItems.join(', ')}
            </div>
          </div>
          <div>
            ...
            <ul className="space-y-1 list-disc list-inside text-sm">
              {analysis.build.situationalItems.map(item => (
                <li key={item.item}>
                  <strong className="text-text-primary">{item.item}:</strong>{' '}
      ...
      <Section title="Composition">
        <div className="space-y-2 text-sm">
          <p>
            <strong className="font-semibold text-text-primary">Ideal Archetypes:</strong>{' '}
            {analysis.composition.idealArchetypes.join(', ')}
          </p>
          <p>
            <strong className="font-semibold text-text-primary">Synergies:</strong>{' '}
            {analysis.composition.synergisticChampions.join(', ')}
          </p>
        </div>
      </Section>
    </div>
        ...
        <ul className="space-y-3">
          {matchupAnalysis.strongAgainstTips.map(tip => (
            <li key={tip.championName} className="text-sm">
              <strong className="text-text-primary">{tip.championName}:</strong>{' '}
    ...
    async (signal: AbortSignal) => {
      ...
      try {
        const result = await getMatchupAnalysis(
          champion.name,
          analysis.counters.weakAgainst,
          analysis.counters.strongAgainst,
          signal
        );
        if (!signal.aborted) {
...
export const ChampionDetailModal = ({ isOpen, onClose, champion, onLoadInLab }: ChampionDetailModalProps) => {
  ...
  const { analysis, matchupAnalysis, isLoading, error, fetchAnalysis, fetchMatchups, reset } = useChampionAnalysis(
    champion,
    latestVersion
  );
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'overview':
        return <OverviewDisplay champion={champion} />;
      case 'abilities':
        return <AbilitiesDisplay abilities={champion.abilities} />;
      case 'strategy':
        if (isLoading.analysis) {
          return <SkeletonLoader />;
        }
        if (error.analysis) {
          return <ErrorDisplay message={error.analysis} onRetry={() => fetchAnalysis(new AbortController().signal)} />;
        }
        if (analysis) {
          return <AIStrategyDisplay analysis={analysis} />;
        }
        return null;
      case 'matchups':
        ...
        if (error.matchups) {
          return <ErrorDisplay message={error.matchups} onRetry={() => fetchMatchups(new AbortController().signal)} />;
        }
        if (analysis) {
          return (
            <MatchupsDisplay
              ...
              onRetry={() => fetchMatchups(new AbortController().signal)}
            />
          );
        }
        return null;
      ...
      <>
        ...
        <div className="relative p-6 mt-32">
          <div className="mb-4 border-b border-border flex flex-wrap items-center gap-4">
            ...
            <TabButton active={activeTab === 'matchups'} onClick={() => setActiveTab('matchups')} disabled={!analysis}>
              Matchup Tips
            </TabButton>
          </div>
          ...
          <div className="mt-6 pt-4 border-t border-border flex justify-end">
            <Button
              onClick={() => {
                onLoadInLab(champion.id, champion.roles[0]);
                onClose();
              }}
              variant="primary"
            >
