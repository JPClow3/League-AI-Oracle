...
const ChampionCard = React.memo(
  ({ champion, onSelect }: { champion: ChampionLite; onSelect: (champion: ChampionLite) => void }) => {
    const handleClick = useCallback(() => onSelect(champion), [onSelect, champion]);
    return (
      <button
        onClick={handleClick}
        className="group relative bg-surface border border-border rounded-lg text-left hover:border-accent/80 transition-all duration-300 transform hover:-translate-y-1 overflow-hidden shadow-md hover:shadow-xl hover:shadow-accent/10"
      >
