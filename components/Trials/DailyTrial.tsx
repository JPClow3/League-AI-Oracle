...
export const DailyTrial = ({ navigateToAcademy }: DailyTrialProps) => {
  ...
  useEffect(() => {
    try {
      ...
      // Silently handle cache errors - will fetch fresh data
  ...
  return (
    <div className="space-y-6">
      <div className="bg-surface-primary/50 p-4 rounded-lg shadow-lg text-center border border-border-primary">
        <h1 className="font-display text-3xl font-bold text-text-primary">Daily Challenge</h1>
        <p className="text-sm text-text-secondary">Test your strategic knowledge. A new challenge appears each day.</p>
      </div>
      ...
      <div className="bg-surface-primary p-6 rounded-lg shadow-lg max-w-2xl mx-auto min-h-[300px] flex flex-col justify-center border border-border-primary">
        ...
        {question && !isLoading && (
          <div className="space-y-6">
            ...
            {isAnswered && (
              <div
                className={`p-4 rounded-lg ${selectedAnswer === question.correctAnswer ? 'bg-success/10' : 'bg-error/10'}`}
              >
                <h3
                  className={`font-bold text-lg ${selectedAnswer === question.correctAnswer ? 'text-success' : 'text-error'}`}
                >
