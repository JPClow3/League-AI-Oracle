...
import { Champion, DDragonData, DraftState, Team, AIAnalysis, Aura, DraftHistoryEntry, Role, View, AIChat, SharePayload, CounterIntelligence } from '../types';
...
import { CounterIntelToast } from './common/CounterIntelToast';
...
const DraftingScreen: React.FC<DraftingScreenProps> = ({ ddragonData, setAura, setView, setHoverAura }) => {
  ...
  const [counterIntel, setCounterIntel] = useState<{ championName: string; data: CounterIntelligence } | null>(null);
  const [showIntelToast, setShowIntelToast] = useState(false);
    ...
    if (newRedPicks.length > lastSeenRedPickObjects.length) {
        ...
        if (newPick.champion) {
            const fetchIntel = async () => {
                ...
                if (intel) {
                    setCounterIntel({ championName: newPick.champion!.name, data: intel });
                    setShowIntelToast(true);
                }
            };
            fetchIntel();
        }
  ...
  }, [draftState.redTeam.picks, settings]);
  ...
  return (
    <div className="space-y-6">
        {showIntelToast && counterIntel && (
            <CounterIntelToast
                championName={counterIntel.championName}
                intel={counterIntel.data}
                onClose={() => setShowIntelToast(false)}
            />
        )}
            ...
            <div className="space-y-6">
                <div className="p-4 bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <ChampionGrid
                        ...
                        highlightArchetypes={counterIntel?.data.counterArchetypes ?? null}
