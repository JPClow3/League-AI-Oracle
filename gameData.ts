...
// This is a sample list, enhanced with class and subclass information where appropriate,
// based on common LoL classifications and the provided documents.
// DDragon keys are typically the champion name, but some differ (e.g., MonkeyKing for Wukong).
// **NOTE**: For the TeamAnalyticsDashboard to be fully accurate, the new fields 
// (damageType, ccTypes, engagePotential, teamArchetypes) need to be populated for ALL champions.
// Only a few examples are populated below for demonstration.
export const staticChampionData: ChampionStaticInfo[] = [
  { 
  ...
  { name: "Anivia", primaryRole: "Mid", championClass: "Mage", championSubclass: "Battle Mage", ddragonKey: "Anivia", damageType: "Magic", ccTypes: ["Stun", "Slow", "Wall"], engagePotential: "Low", teamArchetypes: ["Control", "Siege", "Zone"] },
  ...
  { name: "Aurora", primaryRole: "Top", championClass: "Fighter", championSubclass: "Skirmisher", ddragonKey: "Aurora", damageType: "Mixed" }, // New champ, needs full data
...
/**
 * Provides a summarized string of champion primary roles and classes for system prompts.
 * @param Slices the list to this many champions to keep prompt short.
 * @returns A string like "Aatrox (Top, Fighter), Ahri (Mid, Mage), ..., and others."
 */
export const getStaticChampionRoleSummary = (limit: number = 5): string => {
  if (staticChampionData.length === 0) return "No static champion data available.";

  const effectiveLimit = Math.min(limit, staticChampionData.length);
  let summary = staticChampionData
    .slice(0, effectiveLimit)
    .map(c => {
      let base = `${c.name} (${c.primaryRole}, ${c.championClass || 'N/A'}`;
      if (c.damageType) base += `, Dmg: ${c.damageType}`;
      if (c.engagePotential) base += `, Engage: ${c.engagePotential}`;
      base += ')';
      return base;
    })
    .join('; ');

  if (staticChampionData.length > effectiveLimit) {
    summary += '; and many others with defined roles, classes, subclasses, damage types, and engage potentials.';
  } else {
    summary += '.';
  }
  return summary;
...
/**
 * Retrieves a champion's static data by their DDragon ID or name.
 * Prefers DDragon ID for matching.
 * @param {string} identifier - The DDragon ID (e.g., "Aatrox", "MonkeyKing") or champion name.
 * @returns {ChampionStaticInfo | undefined} The champion's static info, or undefined if not found.
 */
export const getChampionStaticInfoById = (identifier: string): ChampionStaticInfo | undefined => {
  ...
  let champion = staticChampionData.find(c => c.ddragonKey?.toLowerCase() === lowerId);
  if (!champion) {
    champion = staticChampionData.find(c => c.name.toLowerCase() === lowerId);
  }
  return champion;
};
